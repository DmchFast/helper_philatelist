...
function AlbumStampsTable({ stamps = [], onEditStamp, onDeleteStamp }) {
  ...
  const handleSaveEdit = (updatedStamp) => {
    if (onEditStamp) onEditStamp(updatedStamp)
    setEditOpen(false)
    setSelectedStamp(null)
  }

  const handleConfirmDelete = () => {
    if (onDeleteStamp && selectedStamp) onDeleteStamp(selectedStamp.id)
    setDeleteOpen(false)
    setSelectedStamp(null)
  }
  ...
  return (
    <div className="album-stamps">
      <div className="album-stamps__scroll">
          ...
          {stamps.length === 0 ? (
            ...
            stamps.map((stamp) => (
              <div key={stamp.id} className="album-stamps__row">
                <div className="album-stamps__cell album-stamps__cell--stamp" data-label="Марка">
                  ...
                  {/* Новый блок – будет виден только на мобильных */}
                  <div className="album-stamps__title-block">
                    <Text className="album-stamps__name">{stamp.title}</Text>
                    <div className="album-stamps__meta">
                      {stamp.country} • {stamp.series}
                    </div>
                  </div>
                </div>

                <div className="album-stamps__cell" data-label="Информация">
                ...
                <div className="album-stamps__cell" data-label="Цена">
                  <span className="album-stamps__price">{stamp.price} ₽</span>
                </div>

                <div className="album-stamps__cell" data-label="Год">
                  <Text className="album-stamps__year">{stamp.year}</Text>
                </div>

                <div className="album-stamps__cell album-stamps__actions" data-label="Действия">
                  <button
                    type="button"
      ...
      <AlbumDetailModal open={detailOpen} stamp={selectedStamp} onClose={() => { setDetailOpen(false); setSelectedStamp(null) }} />
      <EditStampModal open={editOpen} stamp={selectedStamp} onCancel={() => { setEditOpen(false); setSelectedStamp(null) }} onSave={handleSaveEdit} />
      <DeleteConfirmModal open={deleteOpen} onCancel={() => { setDeleteOpen(false); setSelectedStamp(null) }} onConfirm={handleConfirmDelete} title={`марку "${selectedStamp?.title}"`} />
    </div>
  )
}
